###################################################
###  PEPRMT Methane V-7.3.1: Sulfate Inhibition   ### 
###################################################
...
# What is new in V7.3 model?
#1. 7.3 maintains Debjani Sihi's Sulfate inihibition DAMM model
#   but removes the Mikalis-Menten Equation for Oxygen.

#2. Water density calculations were corrected by adding
#   parentheses around significans.

#3. The inclusion of a new a for-loop that allows to run the methane component
#   multiple times for a data set that contain data from different localities.
#   - the for loop iterates based on a numerical counter that equates to location.

# changes mad in V7.3 created by Dr. Patty Oikawa in collaboration
# with Dr. James Holmquist

#Inputs: 
#1. Theta: a vector of 7 important inputs to PEPRMT CH4.
#            these should come from FME.

#2. Data: a data frame containing 9 variables as described at start of function.
 
#3. Wetland_type
# - "wetland_type == 1" corresponds to a "peatland wetland", which activates
#     the % reduction loop. 
...
CH4_daily_step <- function(theta,
  ...
  #currently not using O2 in DAMM but may add back in later
  ...
  for(i in 1:length(q)){
    ...
    #Exogenous Variables
    Time_2 = d[,1] # day of year
    DOY_disc_2=d[,2] #day of year that starts over every year
    TA_2 = d[,3] #Air temperature- measured (C)
    WT_2 = d[,4] #water table height (cm) equals 0 when water table at soil surface
    GPP_2 = d[,5] * -1 #Modeled GPP - use output from PEPRMT-GPP (gC m-2 day-1) where - = uptake
    wetland_age_2= d[,6]
    Sal <- d[,7]
    S1_2 = d[,10]#Modeled SOC pool - use output from PEPRMT Reco model -cummulative grams m-2
    S2_2 = d[,11]#Modeled labile C pool - use output from PEPRMT Reco model -cummulative grams m-2
  ...
  for(t in 1:length(Time_2)) {
    ...
    # g salt/kg seawater * kg seawater/m^3 seawater * g sulfate/g salt * 1000 mg sulfate/ g sulfate * m^3 seawater/1000L seawter 
    #THIS EQUATION DOESNT WORK SAL CANCELS OUT AND CONCENTRATION SAME NO MATTER WHAT SALINITY IS
    #conc_so4AV = Sal[t] * seawater_density * sul_ratio * 1000 *1/1000 # mg sulfate/L seawater (Typical seawater 2800 mg SO4/L) 2.6 g/kg
   
    conc_so4AV=7.4e-8*(Sal[t]*1e6)*1000 # ppm or mg L-1
    #add in additional sensitivity to a threshold 12ppt identified by Sarah R.
    #if (Sal[t]>12 & Sal[t]<=20) {conc_so4AV=conc_so4AV*2}
    #if (Sal[t]>20 & Sal[t]<=30) {conc_so4AV=conc_so4AV*20}
    #if (Sal[t]>30) {conc_so4AV=conc_so4AV*30}
