...
ss <- function(theta,
  ...
  #6. calculate sample size othetaf non nan observations
  ...
  #Put extra weight on pulses TURNED OFF FOR NOW
  ss2<- ss1
  ydata_ave <- mean(ydata, na.rm = TRUE)
  
  for(i in 1:length(ydata)){
    if(ydata[i] > 4*ydata_ave){ss2[i] = ss1[i]*1}
    else {ss2[i] = ss1[i]}
  }
