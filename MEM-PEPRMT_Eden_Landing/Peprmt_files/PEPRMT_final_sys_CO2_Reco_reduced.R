...
PEPRMT_final_sys_CO2_Reco <- function(data,
  ...
  #Exogenous Variables
   ...
   GPP_2 <- xdata[,5] #gpp modeled--g C m-2 d-1 (negative for uptake)
   GI_2 <- xdata[,6]#xdata[,8] #greeness index from Phenocam or Landsat etc
   Season_drop_2 <- xdata[,7]#xdata[,9] #Season variable that is set to 1 in winter (DOY 1-88, 336-365), 2 pre-spring (DOY 89-175), 3 spring (DOY 176-205), 4 summer (DOY 206-265), 5 fall (DOY 266-335)
   #wc_90CI_2 <- xdata[,10] # 90 percent confidence interval around NEE determined by gapfilling error and random error
   wetland_age_2 <- xdata[,8] #age in years of wetland; only important if wetland is >4yrs old, otherwise just set to 10
  ...
  C1_init <- theta[5] # all the soil organic matter in top meter from MEM 6.30.20
  # inclusive of current year
  C2_init <- theta[6] #in g C m-3 #all labile carbon in top meter from MEM 6.30.20
  # inclusive of current year                    
