...
#3. The creation of a gamma PDF distribution for the combined carbon pool.
# - this pool is randomly sampled at the beginning of each
#   iteration based on the number of rows in the inputted
#   data, with replacement.
# - The parameters of theta(beta in R) and alpha are based on the mean of the SOC and Labile
#   pools and a unique Stand. Dev. value that was calculated by
#   comparing combined(S1 +S2) carbon average vs. the average
#   carbon measured across 5 averaged sediment cor carbon
#   measures. the difference between the two was added to the stand.
#   deviation of each pool separately.
...
CH4_daily_step <- function(theta,
  ...
  for(i in 1:x){
    ...
    #S1 <- rnorm(length(Time_2), mean = mean(S1_2), sd = mean(S1_2) + .05*mean(S1_2))
    #S2 <- rnorm(length(Time_2), mean = mean(S2_2), sd = mean(S2_2)+ .05*mean(S2_2))   
  ...
  ch4_output<- as.data.frame(cbind(uu,y,v,w,s))
